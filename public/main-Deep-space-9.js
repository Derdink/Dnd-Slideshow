...
if (document.getElementById('settingsForm')) {
    ...
    let currentPage = 1; // assume currentPage is defined globally
    let totalPages = 1; // assume totalPages is defined globally
    let currentLimit = 50; // default results per page
    ...
    // UPDATED: fetchImages now extracts the images array from the response
        ...
        fetch(`/api/images?${query.toString()}`)
            ...
            .then(data => {
                ...
                if (data.hasOwnProperty('images')) {
                    ...
                    images = data;
                    totalPages = 1; // Default to 1 if no pagination info is provided
                }
...
// Ensure you have global variables currentPage and totalPages defined
let currentPage = 1;
let totalPages = 1;
let currentLimit = 50; // default results per page

// Add updatePageInfo function definition
...
// Also update fetchImages to use currentLimit:
function fetchImages() {
    const searchVal = document.getElementById('search').value.trim().toLowerCase();
    const query = new URLSearchParams({
        page: currentPage,
